...
export function RunButton({ initialRunning = false }: { initialRunning?: boolean }) {
  ...
  async function start() {
    ...
    setPhase("starting");
    try {
      const res = await fetch(`${API_BASE}/api/run?skip_ingest=true`, { method: "POST" });
      ...
      if (!body.started && body.reason && body.reason !== "already running") throw new Error(body.reason);
      setPhase("running");
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
      setPhase("error");
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      >
        ...
        {phase === "starting" ? "Starting…" : phase === "running" ? "Pipeline running…" : "Run pipeline"}
      </button>
    </div>
  );
}
